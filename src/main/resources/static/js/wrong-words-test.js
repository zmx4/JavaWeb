$(function () {
    const $projectSelect = $('#wtProjectSelect');
    const $startBtn = $('#wtStartBtn');
    const $submitBtn = $('#wtSubmitBtn');
    const $resetBtn = $('#wtResetBtn');
    const $questionList = $('#wtQuestionList');
    const $statusBar = $('#wtStatusBar');
    const $resultPanel = $('#wtResultPanel');
    const $workspace = $('#wtWorkspace');
    const $intro = $('#wtIntro');
    ...
    function renderQuestions(questions) {
        if (!questions || questions.length === 0) {
            ...
            setStatus('错题本中没有可用题目，请先做几次测试。', 'danger');
            return;
        }
        ...
        setStatus('已加载 ' + questions.length + ' 道错题题目，开始作答吧。', 'success');
        $resultPanel.hide().empty();
    }

    function loadQuestions() {
        const project = $projectSelect.val();
        setStatus('正在加载错题题目…', 'info');
        $submitBtn.prop('disabled', true);

        $.getJSON('/test/wrong-words-test/questions', {project: project, count: questionCount})
    ...
    function renderResult(response) {
        ...
        const html = `
            ...
            <div class="wt-actions" style="margin-top:0.75rem;">
                <a href="/test/wrong-words-test" class="btn primary">再来一次</a>
                <a href="/test/wrong-words" class="btn ghost">查看错题本</a>
            </div>
        `;
    ...
    function submitTest() {
        ...
        $.ajax({
            url: '/test/wrong-words-test/submit',
            method: 'POST',
            contentType: 'application/json; charset=UTF-8',
            data: JSON.stringify({
                project: $projectSelect.val() || 'CET4',
                answers: answers
            })
        }).done(function (response) {
            if (response.code === 200) {
